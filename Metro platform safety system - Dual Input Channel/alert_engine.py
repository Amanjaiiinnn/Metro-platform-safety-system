...
class AlertEngine:
    """Terminal alert manager with a PER-CAMERA cooldown and a single,
    strictly serialized audio channel.

    Within one camera the cooldown is global on purpose: if multiple
    people cross that camera's line in the same frame, only the first
    accepted crossing prints an alert.

    ACROSS cameras the cooldowns are independent -- cam2 watches a
    different stretch of platform, so someone stepping over cam2's line
    must still be warned even if cam1 alerted a second ago. What is NOT
    independent is playback: every camera's alerts go through the one
    queue drained by the one audio worker below, so two alerts can never
    be spoken over the top of each other. A cam2 alert raised mid-clip
    waits its turn and plays immediately after cam1's finishes.

    Per-camera state (cooldown clock, "is someone still in the zone")
    is keyed by camera id -- "cam1" / "cam2" -- with cam1 as the default
    so a single-camera setup behaves exactly as it did before.
    ...
    def __init__(
        ...
        # cam id -> time.monotonic() of that camera's last accepted alert.
        # Missing key == never alerted, which can_trigger() treats as
        # "cooldown long expired".
        self._last_alert_time = {}
        self._tts_queue = None
        self._warned_missing = set()

        # cam id -> whether a person is currently inside THAT camera's
        # alert zone this frame. Updated live, every processed frame, from
        # main.py's handle_alert_state()/handle_person_detection_events()
        # -- same "plain dict, no lock" pattern video_widget.py already
        # uses for active_languages/volume/cooldown_seconds (safe because
        # CPython dict item assignment is atomic). The audio worker below
        # reads the JOB'S OWN camera between languages so it can stop
        # playing further languages the moment that person steps back,
        # instead of unconditionally finishing every checked language
        # regardless of whether anyone is still there to hear it. Reading
        # the right camera matters: cam1 emptying out must not cut cam2's
        # alert short, and vice versa.
        self._person_in_zone = {}

        # Guards the pending-job bookkeeping below. The backlog-collapse
        # rule in _queue_alert_job() is per camera, so it needs to know
        # how many of THIS camera's jobs are still waiting -- qsize()
        # alone can't tell cam1's queued alert from cam2's.
        self._queue_lock = threading.Lock()
        self._pending_by_cam = {}
    ...
    # ------------------------------------------------------------------
    def set_person_in_zone(self, cam: str, present: bool) -> None:
        self._person_in_zone[cam] = bool(present)

    def is_person_in_zone(self, cam: str) -> bool:
        return bool(self._person_in_zone.get(cam, False))

    def can_trigger(self, cam: str = "cam1") -> bool:
        last = self._last_alert_time.get(cam)
        if last is None:
            return True
        return (time.monotonic() - last) >= self.cooldown_seconds

    @staticmethod
    def _cam_label(cam: str) -> str:
        return CAMERA_LABELS.get(cam, str(cam))
    ...
    def trigger(
        ...
        if not ignore_cooldown and not self.can_trigger(cam):
            return False

        self._last_alert_time[cam] = time.monotonic()
        ...
        # language checkboxes are on. The camera label is on it so a
        # two-camera log can be read at a glance.
        print("-" * 60, flush=True)
        print(f"{timestamp} [{self._cam_label(cam)}] Message : {message}", flush=True)
        print("-" * 60, flush=True)

        self._queue_alert_job(general=False, color=color, garment=garment, cam=cam)
        return True

    def trigger_general(self, ignore_cooldown: bool = False, cam: str = "cam1") -> bool:
        ...
        Shares that camera's cooldown with trigger() (both write/read the
        same _last_alert_time[cam]) -- a general alert doesn't get to jump
        the queue ahead of the normal cooldown just because more people
        are involved. The OTHER camera's cooldown is untouched.
        """
        if not ignore_cooldown and not self.can_trigger(cam):
            return False
        ...
        self._queue_alert_job(general=True, cam=cam)
        return True
    ...
    def _queue_alert_job(self, general: bool, color: str = None, garment: str = None,
        ...
        Crucially the backlog is counted PER CAMERA. A cam1 alert waiting
        behind a cam2 alert is not a cam1 backlog: those are two different
        people at two different places on the platform, and collapsing
        either into "multiple people" because the other one exists would
        be plainly wrong. Only a camera stacking up its own alerts means
        the description can no longer be trusted to match whoever is
        actually still standing there.
        """
        ...
        with self._queue_lock:
            if self._pending_by_cam.get(cam, 0) >= 1:
                general = True
                color = None
                garment = None
                self._drain_pending_jobs(cam)

            self._pending_by_cam[cam] = self._pending_by_cam.get(cam, 0) + 1
            self._tts_queue.put(
                {"general": general, "color": color, "garment": garment, "cam": cam}
            )

    def _drain_pending_jobs(self, cam: str):
        """Discard `cam`'s jobs that are still WAITING in the queue (not
        the one currently in flight, if any -- the worker already removed
        that one from the queue before this can run). Used right before
        collapsing to a general alert so a stale, superseded specific
        description never gets played after the general one.

        Other cameras' queued jobs are drained out and put straight back,
        in their original order, so this camera's cleanup never swallows
        an alert that belongs to the other camera's zone.
        ...
        while True:
            try:
                ...
                break
            if job.get("cam", "cam1") != cam:
                keep.append(job)
        for job in keep:
            self._tts_queue.put(job)
        self._pending_by_cam[cam] = 0
    ...
    def _start_audio_worker(self):
        ...
        def worker():
            while True:
                ...
                cam = job.get("cam", "cam1")
                with self._queue_lock:
                    self._pending_by_cam[cam] = max(
                        0, self._pending_by_cam.get(cam, 0) - 1
                    )
                ...
                for language in list(self.active_languages):
                    ...
                    # START the next one.
                    #
                    # Checked against THIS job's own camera: the other
                    # camera's zone emptying out says nothing about
                    # whether the person this alert is about is still
                    # there.
                    if not self.is_person_in_zone(cam):
                        break
                    if is_general:
