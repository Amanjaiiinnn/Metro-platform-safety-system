    ...
    def start(self, widget):
        ...
        if(self.platform == "i.MX93" and backend == "NPU"):
            ...
            if not os.path.exists(MODEL_DIR + yolo_vela_model):
                ...
                subprocess.run(
                    f"vela {MODEL_DIR}{self.yolo_model} --output-dir={MODEL_DIR}",
                    shell=True,
                    check=True,
                )
            ...
            if not os.path.exists(MODEL_DIR + clothing_vela_model):
                ...
                subprocess.run(
                    f"vela {MODEL_DIR}{self.clothing_model} --output-dir={MODEL_DIR}",
                    shell=True,
                    check=True,
                )
            ...
            if not os.path.exists(MODEL_DIR + pose_vela_model):
                ...
                subprocess.run(
                    f"vela {MODEL_DIR}{self.pose_model} --output-dir={MODEL_DIR}",
                    shell=True,
                    check=True,
                )
