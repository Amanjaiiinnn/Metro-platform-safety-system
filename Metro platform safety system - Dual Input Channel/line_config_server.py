...
# ---------------------------------------------------------------------
# Config schema -- PER-CAMERA vs GLOBAL
# ---------------------------------------------------------------------
# Everything that is physically tied to one camera's own view -- the zone
# line, its alert side, the proximity threshold, the capture resolution
# those coordinates are expressed in, and the image adjustments -- lives
# in its own block under config.json's "cameras" key:
#
#   {"cameras": {"cam1": {...}, "cam2": {...}}, <global keys...>}
#
# Two cameras point at different stretches of platform under different
# lighting, so none of those can be shared. Everything else (detection
# confidences, alert cooldown/volume/languages, the train schedule) is
# one system-wide setting and stays at the top level.
#
# read_camera_config(cam) flattens one camera's block back over the
# globals, producing exactly the old single-camera dict shape -- which is
# what ProximityDetector and apply_image_adjustments still consume, so
# neither of them had to learn about cameras at all.
# ---------------------------------------------------------------------
...
_CAMERA_KEYS = tuple(_DEFAULT_CAMERA_CONFIG)

_DEFAULT_GLOBAL_CONFIG = {
    # Whether the second camera participates at all. Toggled live from the
    # dashboard; video_widget picks the change up without a restart.
    "camera2_enabled": bool(CAMERA_2_ENABLED),
    # Detection thresholds
    "person_conf_th": 0.50,
    "train_conf_th": 0.45,
    # Alert / audio
...
_DEFAULT_CONFIG = {
    **_DEFAULT_GLOBAL_CONFIG,
    "cameras": {cam: dict(_DEFAULT_CAMERA_CONFIG) for cam in CAMERA_IDS},
}
...
def _migrate_config(raw: dict) -> dict:
    """Normalize whatever's on disk into the two-camera schema.

    Handles both shapes without asking which one it's looking at:
      * NEW  -- already has a "cameras" block; unknown keys inside each
                camera are dropped, missing ones filled from defaults.
      * OLD  -- the pre-cam2 flat file, where p1/p2/alert_side_pt/
                threshold/frame_* and the image adjustments sat at the top
                level. Those become cam1's block (that camera IS the one
                the operator already lined up), and cam2 starts from
                defaults. This runs on every load, so an existing
                single-camera config.json keeps working untouched and
                gets upgraded in place the first time anything is saved.
    Anything unparseable falls back to defaults rather than raising --
    a corrupt config must never stop the dashboard from coming up.
    """
    if not isinstance(raw, dict):
        return _blank_config()

    cfg = {key: raw.get(key, default) for key, default in _DEFAULT_GLOBAL_CONFIG.items()}

    # Legacy flat per-camera keys -> cam1.
    legacy = {key: raw[key] for key in _CAMERA_KEYS if key in raw}
    stored = raw.get("cameras")

    cameras = {}
    for cam in CAMERA_IDS:
        block = dict(_DEFAULT_CAMERA_CONFIG)
        if cam == "cam1" and legacy:
            block.update(legacy)
        if isinstance(stored, dict) and isinstance(stored.get(cam), dict):
            block.update({k: v for k, v in stored[cam].items() if k in _CAMERA_KEYS})
        cameras[cam] = block

    cfg["cameras"] = cameras
    cfg["camera2_enabled"] = bool(cfg.get("camera2_enabled", CAMERA_2_ENABLED))
    cfg["tts_languages"] = _sanitize_languages(cfg.get("tts_languages"))
    cfg["alert_volume"] = _sanitize_volume(cfg.get("alert_volume"))
    return cfg


_active_config = _blank_config()
    ...
    try:
        if os.path.exists(CONFIG_PATH):
            with open(CONFIG_PATH) as f:
                _active_config = _migrate_config(json.load(f))
        else:
            _active_config = _blank_config()
    except Exception:
        _active_config = _blank_config()
...
def _read_config() -> dict:
    """Full config, cameras block included. The nested per-camera dicts
    are copied too -- a shallow dict() would hand callers the live inner
    dicts, and /get_lines rescales those coordinates in place."""
    cfg = dict(_active_config)
    cfg["cameras"] = {
        cam: dict(block) for cam, block in _active_config.get("cameras", {}).items()
    }
    return cfg


def read_camera_config(cam: str = "cam1") -> dict:
    """One camera's settings in the FLAT, pre-cam2 dict shape: every
    global key with that camera's own block merged over the top.

    This is the only view ProximityDetector and
    video_widget.apply_image_adjustments ever see, which is why neither
    needed to change: to them a camera's config still looks like the
    single-camera config.json always did. An unknown cam id falls back to
    cam1 rather than returning an empty dict -- a caller with a bad id
    should behave like the primary camera, not silently lose its line.
    """
    cfg = dict(_active_config)
    cameras = cfg.pop("cameras", {})
    block = cameras.get(cam) or cameras.get("cam1") or _DEFAULT_CAMERA_CONFIG
    cfg.update(block)
    cfg["camera_id"] = cam
    return cfg


def camera2_enabled() -> bool:
    """Live "is cam2 switched on" flag, read every frame by the capture
    loop so the dashboard toggle takes effect without a restart."""
    return bool(_active_config.get("camera2_enabled", CAMERA_2_ENABLED))
...
_current_resolution = {cam: {"width": None, "height": None} for cam in CAMERA_IDS}


def set_frame_resolution(width: int, height: int, cam: str = "cam1") -> None:
    """Record the live capture resolution of ONE camera. Two cameras can
    negotiate different modes, so each camera's stored line coordinates
    are rescaled against its own resolution, never the other's."""
    with _resolution_lock:
        if cam not in _current_resolution:
            return
        _current_resolution[cam]["width"] = int(width)
        _current_resolution[cam]["height"] = int(height)


def _get_current_resolution(cam: str = "cam1"):
    with _resolution_lock:
        entry = _current_resolution.get(cam) or {}
        return entry.get("width"), entry.get("height")
...
def _rescale_camera_block(block: dict, width: int, height: int) -> None:
    """Rescale one camera's stored line geometry to its CURRENT live
    resolution, in place. Same maths as before, just applied per camera
    against that camera's own capture size."""
    old_w = block.get("frame_width") or width
    old_h = block.get("frame_height") or height
    if old_w and old_h and (old_w, old_h) != (width, height):
        sx = width / old_w
        sy = height / old_h
        block["p1"] = [block["p1"][0] * sx, block["p1"][1] * sy]
        block["p2"] = [block["p2"][0] * sx, block["p2"][1] * sy]
        block["alert_side_pt"] = [
            block["alert_side_pt"][0] * sx,
            block["alert_side_pt"][1] * sy,
        ]
        block["threshold_pixels"] = block.get("threshold_pixels", 0.0) * ((sx + sy) / 2.0)
...
def get_lines():
    ...
    for cam in CAMERA_IDS:
        block = data["cameras"].get(cam)
        if not block:
            continue
        width, height = _get_current_resolution(cam)
        if width and height:
            _rescale_camera_block(block, width, height)

    with _health_lock:
        data["camera_status"] = {
            cam: {"status": h["status"], "message": h["message"]}
            for cam, h in _camera_health.items()
        }
...
def camera_status():
    with _health_lock:
        cameras = {cam: dict(h) for cam, h in _camera_health.items()}
    # Top-level status/message stay cam1's, so anything still polling the
    # old flat shape keeps reading the primary camera rather than nothing.
    payload = dict(cameras.get("cam1", {}))
    payload["cameras"] = cameras
    cfg = _read_config()
    payload["camera2_enabled"] = bool(cfg.get("camera2_enabled", CAMERA_2_ENABLED))
...
def _build_config(data: dict) -> dict:
    """Merge an incoming /preview_lines or /update_lines payload over the
    currently active config and return a fully sanitized new config.

    Both routes go through here so the preview path and the saved path can
    never drift apart in what they accept or how they clamp it -- they
    used to be two near-identical copies of the same key list.

    The payload may carry a "cameras" block with either or both cameras.
    A camera the UI didn't send is left exactly as it is: the dashboard
    only ever posts the camera panel(s) it actually has values for, and a
    missing key must not reset the other camera's carefully placed line.
    """
    ...
    config = {
        # Camera 2 on/off
        "camera2_enabled": bool(_get("camera2_enabled", CAMERA_2_ENABLED)),
        # Detection thresholds
        ...
        "alert_volume": _sanitize_volume(_get("alert_volume", _DEFAULT_GLOBAL_CONFIG["alert_volume"])),
        "tts_languages": _sanitize_languages(_get("tts_languages", _DEFAULT_GLOBAL_CONFIG["tts_languages"])),
    ...
    incoming_cams = data.get("cameras")
    if not isinstance(incoming_cams, dict):
        incoming_cams = {}

    cameras = {}
    for cam in CAMERA_IDS:
        current = dict(existing.get("cameras", {}).get(cam) or _DEFAULT_CAMERA_CONFIG)
        payload = incoming_cams.get(cam)
        if not isinstance(payload, dict):
            cameras[cam] = current  # untouched by this request
            continue

        def _cam_get(key, default, _payload=payload, _current=current):
            return _payload.get(key, _current.get(key, default))

        # The live capture resolution wins over whatever the browser
        # reported -- the canvas may still be showing a stale size.
        width, height = _get_current_resolution(cam)
        cameras[cam] = {
            # Line geometry (stored in frame-space coordinates)
            "p1":             _cam_get("p1",            _DEFAULT_CAMERA_CONFIG["p1"]),
            "p2":             _cam_get("p2",            _DEFAULT_CAMERA_CONFIG["p2"]),
            "alert_side_pt":  _cam_get("alert_side_pt", _DEFAULT_CAMERA_CONFIG["alert_side_pt"]),
            "threshold_pixels": float(_cam_get("threshold_pixels", 0.0)),
            "frame_width":  width  or int(_cam_get("frame_width",  _DEFAULT_CAMERA_CONFIG["frame_width"])),
            "frame_height": height or int(_cam_get("frame_height", _DEFAULT_CAMERA_CONFIG["frame_height"])),
            # Image adjustments (all integers, -100..100)
            "brightness": int(_cam_get("brightness", 0)),
            "contrast":   int(_cam_get("contrast",   0)),
            "exposure":   int(_cam_get("exposure",   0)),
            "saturation": int(_cam_get("saturation", 0)),
            "gamma":      int(_cam_get("gamma",      0)),
        }
...
def preview_lines():
    ...
    """
    global _active_config
    _active_config = _build_config(request.json or {})
...
# One preview queue per camera -- a shared queue would interleave cam1 and
# cam2 JPEGs into whichever MJPEG response happened to pull next, so each
# feed would flicker between the two views.
_frame_queues = {cam: queue.Queue(maxsize=2) for cam in CAMERA_IDS}
# Kept as a module-level alias for cam1's queue: this name was part of the
# module's surface before cam2 existed.
frame_queue = _frame_queues["cam1"]

_health_lock = threading.Lock()
_camera_health = {
    cam: {"status": "live", "message": "Live", "last_update": 0.0}
    for cam in CAMERA_IDS
}


def set_camera_health(status_code: str, message: str, cam: str = "cam1") -> None:
    with _health_lock:
        entry = _camera_health.get(cam)
        if entry is None:
            return
...
def update_frame(frame, cam: str = "cam1") -> None:
    """Push a frame to ONE camera's Flask live-preview MJPEG stream.
...
def _generate_mjpeg(cam: str = "cam1"):
    q = _frame_queues.get(cam) or _frame_queues["cam1"]
    while True:
        frame = q.get()
...
def video_feed2():
    """Cam2's live preview. A separate static route rather than
    /video_feed/<cam> so it can't be swallowed by the static_files
    catch-all, and so cam1's URL stays exactly what it always was."""
    return Response(
        _generate_mjpeg("cam2"),
        mimetype="multipart/x-mixed-replace; boundary=frame",
    )
