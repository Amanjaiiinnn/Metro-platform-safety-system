...
try:
    from line_config_server import read_camera_config as _server_read_config
except ImportError:
...
class ProximityDetector:
    """One instance per camera -- each camera has its own zone line, alert
    side and proximity threshold, so cam1 and cam2 never share a detector.
    `cam_id` selects which block of config.json's "cameras" this instance
    reads; everything below still sees the flat, single-camera dict shape
    that read_camera_config() flattens it into.
    """
    ...
    def load(self):
        cfg = _server_read_config(self.cam_id) if _server_read_config is not None else None
        ...
        if cfg is None:
            try:
                ...
                return
            # Standalone/test path (no line_config_server importable):
            # flatten this camera's block over the globals by hand, the
            # same way read_camera_config() would have. An old flat
            # config.json has no "cameras" key and is used as-is.
            cameras = cfg.get("cameras")
            if isinstance(cameras, dict):
                block = cameras.get(self.cam_id) or cameras.get("cam1") or {}
                cfg = {**{k: v for k, v in cfg.items() if k != "cameras"}, **block}
